    ...
    public static class ContainmentBreakerMentalStateUtility
    {
        ...
        public static Pawn FindSCP(Pawn pawn)
        {
            ...
            for (int index = 0; index < allPawnsSpawned.Count; ++index)
            {
                ...
                if (SCP_Startup.IsSCP(pawn1) && pawn1.GetRoom().Role == SCP_Startup.containmentRoom && pawn1.RaceProps.Animal && pawn1 != pawn && !pawn1.IsBurning() && pawn.CanReserveAndReach((LocalTargetInfo)(Thing)pawn1, PathEndMode.Touch, Danger.Deadly))
