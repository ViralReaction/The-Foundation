...
namespace SCP
{
    internal class ThoughtWorker_KittenFlu : ThoughtWorker
    {
        ...
        {
            Hediff firstHediffOfDef = p.health.hediffSet.GetFirstHediffOfDef(HediffDef.Named("SCP_1797_Flu"));
            ...
            {
                PawnUtility.TrySpawnHatchedOrBornPawn(PawnGenerator.GeneratePawn(new PawnGenerationRequest(PawnKindDef.Named("SCP_1797A"), Faction.OfPlayerSilentFail, forceGenerateNewPawn: true, allowDead: true, canGeneratePawnRelations: false, mustBeCapableOfViolence: true, allowGay: false, allowPregnant: true, allowAddictions: false)), (Thing)p);
                SoundDefOf.Hive_Spawn.PlayOneShot((SoundInfo)new TargetInfo(p.Position, p.Map));
                FilthMaker.TryMakeFilth(p.Position, p.Map, ThingDefOf.Filth_AmnioticFluid);
            }
