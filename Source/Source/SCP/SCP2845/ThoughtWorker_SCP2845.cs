...
namespace SCP
{
    internal class ThoughtWorker_SCP2845 : ThoughtWorker
    {
        ...
        {
            Hediff firstHediffOfDef = p.health.hediffSet.GetFirstHediffOfDef(HediffDef.Named("SCP_2845_Transmute_Hediff"));
            if (firstHediffOfDef == null || firstHediffOfDef.FullyImmune())
                return (ThoughtState)false;
            if ((double)firstHediffOfDef.Severity >= 0.8)
            {
                PawnKindDef kindDef = PawnKindDef.Named("SCP_28451R");
                IntVec3 result;
