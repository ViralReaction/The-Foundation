        ...
        private static void AddMenuItems(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
        {
            ...
            if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
            {
                foreach (LocalTargetInfo localTargetInfo in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), true))
                {
                    ...
                    if (SCP_Startup.IsSCP(victim) && !victim.InBed() && pawn.CanReserveAndReach((LocalTargetInfo)(Thing)victim, PathEndMode.OnCell, Danger.Deadly, ignoreOtherReservations: true) && !victim.mindState.WillJoinColonyIfRescued)
                    {
                        ...
                        {
                                Building_Bed targetB = CageUtility.FindBedFor(victim, pawn, false) ?? CageUtility.FindBedFor(victim, pawn, false, true);
                                if (targetB == null)
                                {
